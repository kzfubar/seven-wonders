...
from util.constants import LEFT, RESOURCE_MAP, RIGHT
...
class Action:
    ...
    async def select_card(
        self, player: Player, cards: list[Card], arg: str | None, players: list[Player]
    ) -> Actionable | None:
        pass

    @staticmethod
    async def _get_card(
        player: Player, cards: list[Card], arg: str | None
    ) -> Card | None:
        if arg is None:
        ...
        try:
            ...
            player.display(f"{pos} is out of range!")
            return None

    @classmethod
    async def _select_payment_option(
        cls, player: Player, played_card: Card, payment_options: list[PaymentOption]
    ) -> bool:
        ...
        if bank_payment != 0:
            ...
            if not cls._valid_payment(player, bank_payment):
            ...
            cls._display_payment_options(player, payment_options)
            player.client.clear_message_buffer()
            player.client.send_event(
                GAME,
                {
                    "type": "payment",
                    "options": [i for i, _ in enumerate(payment_options)],
                },
            )
            ...
            try:
                ...
                return False
            try:
                if not cls._valid_payment(player, payment):
                    player.display("Cannot afford this payment")
                    return False
                cls._do_payment(player, payment_options[player_input])
            except Exception:
    ...
    def _display_payment_options(
        ...
        for i, option in enumerate(payment_options):
            ...
            player.display(
                f"({i}) {left.name} <- {option.left_payment}, "
                f"{option.right_payment} -> {right.name} : "
                f"{resources[LEFT]} <---> {resources[RIGHT]}"
            )

    @staticmethod
    def activate_card(player: Player, card: Card) -> None:
        player.add_card_type(card.card_type)
        player.add_coupons({card.name for card in card.coupons})
    ...
    @staticmethod
    def _do_payment(player: Player, payment_option: PaymentOption) -> None:
        left, right = player.get_neighbors()

        left.handle_next_coins(payment_option.left_payment, RIGHT)
        right.handle_next_coins(payment_option.right_payment, LEFT)
        ...
        for lux in payment_option.left_lux:
            left.coins_gained[lux] += payment_option.left_lux_cost
        ...
        for common in payment_option.left_common:
            left.coins_gained[common] += payment_option.left_common_cost
        ...
        for lux in payment_option.right_lux:
            right.coins_gained[lux] += payment_option.right_lux_cost
        ...
        for common in payment_option.right_common:
            right.coins_gained[common] += payment_option.right_common_cost
    ...
    @staticmethod
    def _announce(msg: str, players: list[Player]) -> None:
