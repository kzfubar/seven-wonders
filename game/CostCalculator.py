...
def find_resource_outcomes(left_effects,
                           right_effects,
                           choices,
                           reqs,
                           goods) -> Set[Tuple[Tuple[str], Tuple[str]]]:
    outcomes = list()
    for options in itertools.product([""], *choices):
        ...
        for i in range(1 << len(reqs_curr)):
            ...
            left_purchase, _, updated_left_reqs = simplify_cost_search(left_effects, left_reqs, goods)
            if not valid_resources(left_purchase, updated_left_reqs):
                continue

            right_purchase, _, updated_right_reqs = simplify_cost_search(right_effects, right_reqs, goods)
